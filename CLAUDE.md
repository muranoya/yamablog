...
trail-behind-them（Rust/Axum + SQLite + SolidJS SPA）の後継。サーバーレス構成で同等のブログサービスを提供する。詳細な設計経緯は `docs/superpowers/specs/2026-04-23-yamablog-architecture-design.md` を参照。

## アーキテクチャ概要

```
data/ (JSON + Git)  →  cli/ (Rust ビルドCLI)  →  Cloudflare R2 (静的配信)
                         ↑
...
- サーバー不要。全ファイルをCloudflare R2から静的配信する
...
```sh
yamablog build           # data/ を読んでHTMLを生成してR2にアップロード
...
3. GPXファイルをR2から公開HTTP経由でフェッチし、polylineとbounding boxを計算
...
- `aws-sdk-s3` — R2/S3互換アップロード（エンドポイントURL設定でR2を指定）
- `reqwest` — R2からのGPXファイル取得（公開HTTP経由）
...
ブラウザのCanvas APIで small・medium・original の3サイズにリサイズしてWebPに変換してからR2にアップロードする。EXIFから `shooting_datetime` を抽出する。

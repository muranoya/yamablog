...
use crate::schema::files::{DirectoryFiles, DirectoryFilesItem as ImageFile};
use crate::schema::manifest::ManifestMapMemosItem;

pub(super) type FilesIndex<'a> = HashMap<uuid::Uuid, &'a ImageFile>;
...
pub(super) fn build_content_blocks(
    article: &Article,
    index: &FilesIndex<'_>,
    gpx_cache: &HashMap<String, super::gpx::GpxTrack>,
    ...
    for item in &article.content {
        match item {
            ...
            ArticleContentItem::Image {
                ...
                let medium_src = image_medium_src(index, file_id);
                let display_size = find_display_image_size(index, file_id);
                let description_html = description
                    .as_deref()
                    .map(|d| super::markdown::markdown_to_html(d));
                blocks.push(ContentBlock {
                    kind: "image",
                    html: None,
                    ...
                    stats_json: None,
                    map_memos_json: None,
                    medium_width: display_size.map(|(w, _)| w),
                    medium_height: display_size.map(|(_, h)| h),
                });
            }
            ArticleContentItem::Gpx { filename } => {
                match gpx_cache.get(filename.as_str()) {
                    Some(track) => {
                        ...
                        let gpx_stats = super::gpx::calc_stats(&track.points);
                        let elapsed = match (gpx_stats.start_at, gpx_stats.end_at) {
                            (Some(s), Some(e)) => Some(e - s),
                            _ => None,
                        };
                        let stats_out = GpxStatsOut {
                            elapsed_seconds: elapsed,
                            distance_m: gpx_stats.distance_m,
                            cum_climb_m: gpx_stats.cum_climb_m,
                            cum_down_m: gpx_stats.cum_down_m,
                            max_elevation_m: gpx_stats.max_elevation_m,
                            min_elevation_m: gpx_stats.min_elevation_m,
                        };
                        ...
                        let local_memos: Vec<MapMemoInArticle> = all_map_memos
                            ...
                            .map(|m| MapMemoInArticle {
                                ...
                                image_small_src: m.image_id.as_ref().map(|id| {
                                    image_small_src(index, id)
                                }),
                            })
                            .collect();
                        blocks.push(ContentBlock {
                            ...
                            stats_json: Some(serde_json::to_string(&stats_out)?),
                    ...
                    None => {
                        eprintln!("Warning: GPX not found in cache: {}", filename);
                        blocks.push(ContentBlock {
                            kind: "gpx",
                            html: None,
                            file_id: Some(filename.clone()),
...
fn find_shooting_datetime(index: &FilesIndex<'_>, file_id: &uuid::Uuid) -> Option<String> {
    ...
    let f = index.get(file_id)?;
    let ts = f.shooting_datetime?;
    let dt_jst = DateTime::from_timestamp(ts, 0)?.with_timezone(&jst);
    Some(format!("{}", dt_jst.format("%Y/%m/%d %H:%M")))
}

fn image_medium_src(index: &FilesIndex<'_>, file_id: &uuid::Uuid) -> String {
    match index.get(file_id) {
        Some(f) if f.sizes.medium.is_some() => format!("/images/{}-medium.webp", file_id),
        _ => format!("/images/{}-original.webp", file_id),
    }
}

fn image_small_src(index: &FilesIndex<'_>, file_id: &uuid::Uuid) -> String {
    match index.get(file_id) {
        Some(f) if f.sizes.small.is_some() => format!("/images/{}-small.webp", file_id),
        _ => format!("/images/{}-original.webp", file_id),
    }
}

fn find_display_image_size(index: &FilesIndex<'_>, file_id: &uuid::Uuid) -> Option<(i64, i64)> {
    let f = index.get(file_id)?;
    let size = f.sizes.medium.as_ref().unwrap_or(&f.sizes.original);
    ...
    article
        ...
        .filter_map(|item| {
            ...
            Some(super::gpx::ImageInfo {
                ...
                description: description.clone(),
                small_src: image_small_src(index, file_id),
            })
        })
        .collect()
}
